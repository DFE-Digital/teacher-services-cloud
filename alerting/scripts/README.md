...
eg `./test-logicapp.sh -f metric_alert-sev1_tsc.json`

> NOTE: if copying the Workflow URL for the `CALLBACK_URL` from the portal (or other locations) Mac or Terminal automatically escapes the string for you which make it invalid for use with the test scripts. 
...
Save in the scripts folder naming the file `servicelist.csv`
