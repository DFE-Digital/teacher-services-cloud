...
resource "kubernetes_deployment" "welcome_app_clone" {
  ...
  metadata {
    name      = local.welcome_app_name
    namespace = kubernetes_namespace.default_list_clone[local.welcome_app_namespace].metadata[0].name
  }
  spec {
    replicas = 2
    selector {
    ...
    template {
      ...
      spec {
        ...
        volume {
          name = "nginx-config-volume"
          config_map {
            name = kubernetes_config_map.welcome_app_nginx_config_clone[0].metadata[0].name
          }
        }
...
resource "kubernetes_ingress_v1" "welcome_app_clone" {
  for_each = var.clone_cluster ? toset(local.welcome_app_hostnames) : []
  provider = kubernetes.clone
    ...
    rule {
      ...
      http {
        path {
          backend {
            service {
              name = kubernetes_service.welcome_app_clone[0].metadata[0].name
              port {
                number = kubernetes_service.welcome_app_clone[0].spec[0].port[0].port
              }
            }
          }
        }
