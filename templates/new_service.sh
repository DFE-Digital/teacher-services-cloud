...
EOF
  ...
  cp templates/new_service_config/terraform_env_vars/${envt}.tfvars.json \
    templates/my_service/terraform/application/config
  ...
  cp -r templates/new_service_config/maintenance \
    templates/my_service/maintenance_page/manifests/${envt}
...
' templates/new_service/.github/workflows/maintenance.yml > tmp.yml \
  && mv tmp.yml templates/my_service/.github/workflows/maintenance.yml
...
if [ "${POSTGRES}" != "true" ]; then
  ...
  ' templates/my_service/terraform/application/database.tf
else
  ...
  jq -s '.[0] * .[1]' \
    templates/my_service/terraform/application/config/production.tfvars.json \
  ...
  cat templates/new_service_config/postgres/postgres-vars.tf \
    >> templates/my_service/terraform/application/variables.tf

  #MOVE DB WORKFLOWS
  cp templates/new_service_config/postgres/workflows/* \
    templates/my_service/.github/workflows
fi
...
if [ "${RAILS_APPLICATION}" = "true" ]; then
  perl -pi -e \
    ...
    templates/my_service/terraform/application/application.tf
else
  perl -pi -e \
